...
class ForeignSalesWorker:public SalesWorker
{
	private:
		const int risk;
	public:
		ForeignSalesWorker(char* name,int sale,double rate,int risk):SalesWorker(name,sale,rate),risk(risk){}
		int GetRisk() const
		{
...
int main(void)
{
	...
	ForeignSalesWorker* fseller1=new ForeignSalesWorker("AAA",1000,0.1,RISK_LEV::R1);
	ForeignSalesWorker* fseller2=new ForeignSalesWorker("BBB",1000,0.1,RISK_LEV::R2);
	ForeignSalesWorker* fseller3=new ForeignSalesWorker("CCC",1000,0.1,RISK_LEV::R3);

	fseller1->AddSaleResult(5000);
	fseller2->AddSaleResult(5000);
	fseller3->AddSaleResult(5000);
	handler.AddEmp(fseller1);
	handler.AddEmp(fseller2);
	handler.AddEmp(fseller3);
